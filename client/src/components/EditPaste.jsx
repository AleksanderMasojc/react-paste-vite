...
function EditPaste() {
    ...
    async function download(){
        try{
            ...
            const pattern = "\\n";
            const replacement = "\n";
            setCode(String(JSON.stringify(response.data.paste).substring(1,JSON.stringify(response.data.paste).length-1).replaceAll(pattern, replacement)));
            setLang(String(JSON.stringify(response.data.lang).substring(1,JSON.stringify(response.data.lang).length-1)));
            //console.log(lang);
        } catch(err){
            alert(err);
        }
  ...
  return (
    ...
    <select
    onChange={(e) => setListedS(String(e.target.value))}>
